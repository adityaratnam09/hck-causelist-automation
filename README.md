...
5. **Emails** the report to one or more recipients, but only if the causelist is dated in the future. If the court did not publish a new causelist (weekend or public holiday), the server keeps serving the previous one; the mailer detects this automatically and suppresses the email.
...
```
By default, the mailer will not send an email if the causelist date is today or earlier. 
To override this when testing or to resend a past report:
```bash
...
- **Staleness checks**:the causelist date (extracted from the PDF page 1 header) is embedded in the HTML report as a machine-readable <meta> tag and a visible field in the report header. The mailer reads this date to decide whether to send: causelists dated today or earlier are suppressed automatically; --force overrides this.
